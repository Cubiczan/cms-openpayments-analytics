"""Read-only guardrails for executed answer queries.

The gate never trusts the asking agent: before an answer is promoted its SQL
executes here, against the dbt-built DuckDB file, under four enforcements:
...
4. Row cap — at most ``row_cap`` rows are fetched; exceeding the cap is an
   error, not a silent truncation.

Governance floor: ``ensure_read_only_uri`` fails closed unless the URI is a
DuckDB URI carrying ``access_mode=READ_ONLY``, and the engine connection is
additionally opened ``read_only=True`` — the README's one-writer rule means a
stray write connection would also break every ``dbt build``.
"""
...
# Statements that must never run from the answer path. Token-exact matching, so
# a column named ``description`` or ``created_at`` is not a false positive.
...
SELECT_ONLY_QUERY = "access_mode=READ_ONLY"
...
class GuardrailError(Exception):
    """Base class: a query violated an answer guardrail (or failed under them)."""


class NotReadOnlyUri(GuardrailError):
    """The configured DuckDB URI is not READ_ONLY — the answer path refuses to run."""
...
class RowCapExceeded(GuardrailError):
    ...
    def __init__(self, cap: int) -> None:
        super().__init__(f"query exceeds the answer row cap of {cap} rows")
        self.cap = cap
...
class StatementTimeout(GuardrailError):
    ...
    def __init__(self, timeout_seconds: float) -> None:
        super().__init__(f"query exceeded the answer statement timeout of {timeout_seconds:g}s")
...
def validate_select_only(sql: str) -> str:
    ...
    if not _SELECT_PREFIX.match(stripped):
        raise NotSelectOnly("only SELECT statements are allowed from the answer path")
...
def ensure_read_only_uri(uri: str) -> str:
    ...
    if not uri.startswith("duckdb:///"):
        raise NotReadOnlyUri(f"answers must query DuckDB, got: {uri.split('?')[0]}")
    path, _, query = uri[len("duckdb:///") :].partition("?")
    if SELECT_ONLY_QUERY not in query.replace(" ", ""):
        raise NotReadOnlyUri("answer DuckDB connections must use access_mode=READ_ONLY")
    return path
